...
import { addRecycleEvent } from "@/utils/recycleApi";
 ...
 const handleSubmit = async () => {
  ...
  if (isAuthenticated() && facilityData.length > 0) {
    if (
      ...
      try {
        // Insert all booking fields into Supabase
        const { data: supabaseData, error: supabaseError } = await addRecycleEvent({
          user_id: userId,
          name: recycleItem,
          location: address,
          facility: selectedFacility,
        ...
        console.log('Supabase insert result:', { supabaseError, supabaseData });
        if (supabaseError) {
          toast.error("Error saving to Supabase: " + supabaseError.message, { autoClose: 3000 });
        } else {
